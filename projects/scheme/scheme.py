...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        evalOperator = scheme_eval(first, env)
        if (check_procedure(evalOperator) is None):
            evalOperands = rest.map(lambda fn: scheme_eval(fn,env))
        return scheme_apply(evalOperator, evalOperands, env)
        # END PROBLEM 5
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        while (True):
            if (not (currentFrame is None)):
                ...
                if (symbol in currentFrame.bindings):
                    return currentFrame.bindings[symbol]
    ...
    def make_child_frame(self, formals, vals):
        ...
        for i in range(0,len(formals)):
            newFrame.define(formalsHold.first, valsHold.first)
            formalsHold, valsHold = formalsHold.second, valsHold.second
        return newFrame
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('SchemeError: Incorrect number of arguments.')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        evalExpr = scheme_eval(expressions.second.first, env)
        env.define(target, evalExpr)
        return target
        # END PROBLEM 6
...
def do_and_form(expressions, env):
    ...
    expressionsHold = expressions
    evalHolder = expressions
    # BEGIN PROBLEM 13
    if (len(expressions) == 0):
        return True
    for i in range(0,len(expressions)):
        evalHolder = scheme_eval(expressionsHold.first, env)
        if (not scheme_truep(evalHolder)):
            return False
        expressionsHold = expressionsHold.second
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if (len(clause.second) == 0):
                return test
            retVal = eval_all(clause.second, env)
            return retVal
            # END PROBLEM 14
...
def make_let_frame(bindings, env):
    ...
    check_formals(formals)
    letFrame = env.make_child_frame(formals,values)
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    bodyExpression = expressions.second
    return MuProcedure(formals,bodyExpression)
    # END PROBLEM 16
