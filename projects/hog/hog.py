...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0,score1):
        ...
        checkForUpdate = gain - previous_high 
        newHighest = max(gain,previous_high)
        if checkForUpdate > 0:
            print(gain,'point(s)! That\'s the biggest gain yet for Player',who)
        return announce_highest(who,newHighest,current_score)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    top_avg = 0
    while numRolls <= 10:
        avgTurn = make_averaged(roll_dice, num_samples)
        avg = avgTurn(numRolls, dice)
        if avg > top_avg:
            top_avg = avg
            bestRolls = numRolls
        numRolls += 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    checker = is_swap(score+free_bacon(opponent_score), opponent_score)
    if (checker and score+free_bacon(opponent_score) < opponent_score):
        return 0
    elif  not checker and free_bacon(opponent_score) >= margin:
        return 0
