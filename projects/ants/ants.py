        ...
        if insect.is_ant:
            if (self.ant == None):
                ...
                if (self.ant.can_contain(insect) == True):
                    ...
                    assert self.ant == None, 'Two ants in {0}'.format(self)
                # END Problem 9
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while(place_holder != hive):
            if (place_holder.bees != [] and self.min_range <= self.spot and self.max_range >= self.spot):
                self.spot = 0
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            self_place_copy = self.place
            bee_copy = list(self_place_copy.bees)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if (self.digesting > 0):
            ...
            beeToEat = random_or_none(self.place.bees)
   
            if (beeToEat != None):
                self.eat_bee(beeToEat)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        self.armor = armor
        if (self.unique_queen_born == False):
            self.is_unique = True
            QueenAnt.unique_queen_born = True
    ...
    def action(self, colony):
        ...
        if (self.is_unique == False):
            ...
            place_holder = self.place
            while(place_holder != None):
                    if (place_holder.ant != None and place_holder.ant != self):
                        if (place_holder.ant.is_container == True):
                            if (place_holder.ant.been_buffed == False):
                                place_holder.ant.damage = place_holder.ant.damage*2
                                place_holder.ant.been_buffed = True
                            if (place_holder.ant.contained_ant != None):
                                if (place_holder.ant.contained_ant.been_buffed == False):
                                    place_holder.ant.contained_ant.damage = place_holder.ant.contained_ant.damage * 2
                                    place_holder.ant.contained_ant.been_buffed = True
                        else:
                            if (place_holder.ant.been_buffed == False):
                                place_holder.ant.damage = place_holder.ant.damage*2
                                place_holder.ant.been_buffed = True
    ...
    def reduce_armor(self, amount=0):
